...
- `withdrawals/{id}` — `userId`, `userName`, `userEmail`, `telegramId`, `amount`, `method`, `accountName`, `qrCodeUrl`, `status` (`pending` / `approved` / `rejected` / `binned`), `requestedAt`, `binnedAt`, `processedAt`, `reason`
...
- **Withdraw** → multi-step: amount → method → QR upload → account name → submit
